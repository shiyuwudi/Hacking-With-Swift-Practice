...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let data = ud.objectForKey("people") as? NSData {
            people = NSKeyedUnarchiver.unarchiveObjectWithData(data) as! [Person]
        }
    }
...
//   step4.     Dismiss the view controller. (...)
    ...
    //数据归档
    func save(){
        let data = NSKeyedArchiver.archivedDataWithRootObject(people)
        let ud = NSUserDefaults.standardUserDefaults()
        ud.setObject(data, forKey: "people")
    }
